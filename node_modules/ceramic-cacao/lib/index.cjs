...
(function (Cacao) {
  ...
  const fromBlockBytes = function (bytes) {
    try {
      return Promise.resolve(Block__namespace.decode({
        bytes: bytes,
  ...
  function verifyEIP191Signature(cacao, options) {
    ...
    const recoveredAddress = wallet.verifyMessage(SiweMessage.fromCacao(cacao).toMessage(), cacao.s.s).toLowerCase();
    const recoveredAddresses = [recoveredAddress];

    if (Date.parse(cacao.p.iat) <= LEGACY_CHAIN_ID_REORG_DATE) {
      const legacyChainIdRecoveredAddress = wallet.verifyMessage(asLegacyChainIdString(SiweMessage.fromCacao(cacao), 'Ethereum'), cacao.s.s).toLowerCase();
      recoveredAddresses.push(legacyChainIdRecoveredAddress);
    }

    const issuerAddress = caip.AccountId.parse(cacao.p.iss.replace('did:pkh:', '')).address.toLowerCase();
...
exports.LEGACY_CHAIN_ID_REORG_DATE = LEGACY_CHAIN_ID_REORG_DATE;
exports.SiweMessage = SiweMessage;
exports.SiwsMessage = SiwsMessage;
exports.SiwxMessage = SiwxMessage;
exports.asLegacyChainIdString = asLegacyChainIdString;
